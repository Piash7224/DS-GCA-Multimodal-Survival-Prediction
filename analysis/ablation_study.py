"""
DS-GCA ABLATION CONTROL CENTER (Strict Scientific Baseline Version)

Visual Extractor: AttentionMIL (ABMIL) locked for ALL visual modes.
Includes automated Out-Of-Fold (OOF) prediction tracking and 
patient-level Bootstrap statistical significance testing.
"""
...
#  SEED EVERYTHING (STRICT CONSISTENCY)
...
# EXCLUDE PATIENTS (NOISY / CORRUPTED)
...
class AblationFusionModel(nn.Module):
    def __init__(self, mode, clin_dim=CLIN_EMBED_DIM, vis_dim=VIS_DIM, fusion_dim=FUSION_DIM):
        ...
        # Late Fusion requires two separate classifiers
        if mode == 'LATE_FUSION':
...
def patient_level_bootstrap_test(y_time, y_event, risk_full, risk_baseline, baseline_name, n_bootstraps=1000, alpha=0.05):
    n_patients = len(y_time)
    c_index_diffs = []
    
    print(f"\n Running Bootstrap (N=1000): FULL_DSGCA vs {baseline_name}...")
    np.random.seed(42)
    for i in range(n_bootstraps):
        indices = np.random.choice(n_patients, size=n_patients, replace=True)
        t_boot, e_boot = y_time[indices], y_event[indices]
        r_full_boot, r_base_boot = risk_full[indices], risk_baseline[indices]
        try:
            c_full = concordance_index(t_boot, -r_full_boot, e_boot)
            c_base = concordance_index(t_boot, -r_base_boot, e_boot)
            c_index_diffs.append(c_full - c_base)
        except ZeroDivisionError:
            continue 

    lower_bound = np.percentile(c_index_diffs, (alpha / 2) * 100)
    upper_bound = np.percentile(c_index_diffs, (1 - alpha / 2) * 100)
    
  
    p_value = min(1.0, np.mean(np.array(c_index_diffs) <= 0) * 2)
    
    print("\n" + "="*55)
    print(f" SIGNIFICANCE REPORT: FULL_DSGCA vs {baseline_name}")
    print("="*55)
    print(f"Mean C-Index Difference: +{np.mean(c_index_diffs):.4f}")
    print(f"95% CI of Difference:    [{lower_bound:.4f}, {upper_bound:.4f}]")
    print(f"Empirical p-value:       {p_value:.4f}")
    
    if p_value < alpha and lower_bound > 0:
        print(f" SUCCESS: FULL_DSGCA is significantly better than {baseline_name}.")
    else:
        print(f" DIFFERENCE NOT SIGNIFICANT against {baseline_name}.")
    print("="*55 + "\n")


#  AUTOMATED MASTER EXECUTION
...
if __name__ == "__main__":
    ...
    print("\n" + "#"*50 + "\nSTEP 1: TRAINING FULL_DSGCA MASTER MODEL\n" + "#"*50)
    full_risks = get_oof_predictions('FULL_DSGCA', X_clin_raw, vis_bags, y_time, y_event)
    
   
    baselines_to_test = ['CLINICAL_ONLY', 'VISUAL_ONLY', 'EARLY_FUSION', 'LATE_FUSION', 'GATE_ONLY', 'GCA_NO_GATES']
    
    for baseline in baselines_to_test:
        print("\n" + "#"*50 + f"\nSTEP 2: TRAINING & TESTING BASELINE ({baseline})\n" + "#"*50)
        baseline_risks = get_oof_predictions(baseline, X_clin_raw, vis_bags, y_time, y_event)
        patient_level_bootstrap_test(y_time, y_event, full_risks, baseline_risks, baseline_name=baseline)
        
    print("\n ALL ABLATION EXPERIMENTS & STATISTICAL TESTS COMPLETE! 🎉")